...
// NOTE: Tailwind's own radius/shadow/font-size/font-weight scale classes
// (rounded-md, shadow-sm, text-sm, font-medium...) are NOT flagged below.
// tailwind.config.ts wires theme.extend.{borderRadius,boxShadow,fontSize,
// fontWeight} to CSS custom properties in globals.css (--radius-*,
// --shadow-*, --font-size-*, --font-weight-*), exactly like colors already
// go through --primary etc. So `text-sm` compiles to
// `font-size: var(--font-size-sm)` — it IS the token, not a hardcoded value.
// Flagging the class name itself would also contradict the project's own
// "no Tailwind arbitrary values" rule (the only alternative would be
// text-[var(--font-size-sm)], which is an arbitrary value).
// Numeric spacing utilities (px-4, mt-2, gap-6...) are intentionally left
// as Tailwind's own fixed scale rather than mirrored 1:1 into ~35 CSS vars —
// that scale's whole purpose is to be the consistent, systematic value set.
