...
> **Implementation**: Commandes Claude Code dans `.claude/commands/` + délégation aux sous-agents `.claude/agents/`
...
> **Agent invocation**: Claude Code délègue au sous-agent via l'outil Agent (fichier `.claude/agents/<agent>.md`).
...
# Fail-closed : bloque si REVIEW.md dit "À REVOIR" (voir /.claude/default /ns-review)
# (Le hook réel inclut le contrôle du verdict — voir .husky/pre-push)
...
```bash
# Discovery → sous-agent saas-project-compliance (.claude/agents/saas-project-compliance.md)
